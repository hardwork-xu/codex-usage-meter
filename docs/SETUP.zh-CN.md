...
新用户请先看 [直接通过 Codex 安装](SETUP.github.zh-CN.md)，无需下载源码运行安装器。本文保留已有 `personal` 安装渠道及手动源码安装流程；两种渠道选择一种即可。

适用于 macOS 上的用量计 0.4.0 及之后版本。Windows 请看 [Windows 安装与启用说明](SETUP.windows.zh-CN.md)。**插件安装完成后，自动记录还需要你手动审阅并信任 Hooks。** Hooks 是在开始提问、结束提问等时刻运行的小程序；用量计用它们登记本地统计来源。
...
从 0.7.0 起，macOS 可在浏览器访问时自动启动用量计。在终端执行一次：

```sh
python3 "$HOME/plugins/codex-usage-meter/scripts/browser_access.py" install
```

之后在任意任务的侧边浏览器输入面板原来的地址，即可打开。地址仍以本机保存的值为准，不需要重新发起“打开用量计”的模型请求。统计进程空闲十分钟后退出，再访问会自动唤醒；首次唤醒期间会重新补读历史。此步骤不会代替第 4 步的 Hooks 审阅与信任。

若需关闭此入口，执行以下命令；任务统计和原地址会保留：

```sh
python3 "$HOME/plugins/codex-usage-meter/scripts/browser_access.py" uninstall
```

状态检查及额度查询代理设置见 [README](../README.md#安装与使用)。代理只在确认自己已有的本地 HTTP 代理地址后设置，不要照抄不明端口。
